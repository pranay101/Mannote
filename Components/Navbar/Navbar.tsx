...
const Navbar = (props: Props) => {
    ...
    <nav className='absolute h-36 top-0 left-0 w-screen px-5 md:px-20 text-black flex justify-between items-center z-50 overflow-x-hidden'>
            ...
            <Bars3Icon onClick={() => setOpenMenu(true)} className='md:hidden h-10 w-10 fill-gray-900' />
        </nav>
         <div className={`md:hidden absolute h-screen w-screen bg-gray-50 z-50 ${!openMenu? "translate-x-[100%]" : "translate-x-0" } ease-out duration-500`}>
                <XMarkIcon onClick={() => setOpenMenu(false)} className='h-10 w-10 fill-gray-900 absolute top-10 right-5 z-50 '  />
